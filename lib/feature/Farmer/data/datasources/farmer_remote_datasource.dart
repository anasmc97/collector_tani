...
class FarmerRemoteDataSourceImpl implements FarmerRemoteDataSource {
  @override
  Future<FarmerModel?> addFarmer(String? token, String? name, String? landLocation, String? numberTree, String? estimationProduction, String? landSize) async {
    final api = 'http://192.168.1.7:8000/api/collector/farmer';
    final data = {"name": name, "land_location": landLocation, "number_tree": numberTree, "estimation_production": estimationProduction, "land_size": landSize};
  ...
  Future<List<FarmerModel?>?> getAllFarmer(String? token) async {
    ...
    try{
      ...
      if (response.statusCode == 200) {
        ...
        List<FarmerModel?>? listFarmerModel = (body as Iterable).map((e) => FarmerModel.fromJson(e)).toList();
  ...
  Future<void> deleteFarmer(String? token, String? id) async {
    final api = 'http://192.168.1.7:8000/api/collector/farmer/$id';

    final dio = Dio();
    try{
      await dio.delete(api, options: Options(headers:{"Authorization":"Bearer $token"}));
    }catch (e){
      if (kDebugMode) {
        print(e);
      }
  ...
  @override
  Future<FarmerModel?> updateFarmer(String? token, String? id, String? name, String? landLocation, String? numberTree, String? estimationProduction, String? landSize) async {
    final api = 'http://192.168.1.7:8000/api/collector/farmer/$id';
    final data = {"name": name, "land_location": landLocation, "number_tree": numberTree, "estimation_production": estimationProduction, "land_size": landSize};

    final dio = Dio();
    try{
      await dio.put(api, data: data, options: Options(headers:{"Authorization":"Bearer $token"}));
    }catch (e){
      if (kDebugMode) {
        print(e);
      }
